...
static uint8_t makeConstant(Value value){
    ...
    if(constant > UINT8_MAX) {
        error("Too man constants in one chunk.");
        return 0;
    }
...
static void patchJump(int offset){
    //-2 adjusts for the bytecode jump instruction offset
    int jump = currentChunk()->count - offset - 2;

    if(jump > UINT16_MAX){
        error("jump greater than uint16_t max size.");
    }
...
static int addUpvalue(Compiler* compiler, uint8_t index, bool isLocal){
    ...
    if(upvalueCount == UINT8_COUNT){
        error("Too many closure cariables in functions");
        return 0;
    }
...
static void classDeclaration(){
    ...
    if(match(TOKEN_LESS)){
        ...
        if(identifiersEqual(&className, &parser.previous)){
            error("A class cannor ingerit from itself.");
        }
...
static void ifStatement(){
    consume(TOKEN_LEFT_PAREN, "Expect 'c' after 'if'.");
    expression();
